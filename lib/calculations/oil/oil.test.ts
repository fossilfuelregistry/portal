...
describe('Oil', () => { 
    describe("Combustion emissions", ()=>{
        it("Should calculate the energy from volume",()=> {
            ...
            const barrelsPerTonne = isoBarrelsPerTonne.wrap(7.3300)
            const eiaOilNFURatio = isoEIAOilNFURatio.wrap(0.12)
            const oilMassToEnergyFactor = isoCombustionMassToEnergyFactor.wrap({p5: 40.10, wa:42.30, p95: 44.80 })
            const expectedEnergyOutcome: Scenarios = { p5:3066638 , wa:3234882  ,p95: 3426069  }
            const result = isoEnergyFromOil.unwrap(calculateEnergyFromOil(barrelsPerTonne)(eiaOilNFURatio)(oilMassToEnergyFactor)(oilProduction))
        ...
        it(" 2. Energy -> Emissions ", () => {
            const energyFromOil = isoEnergyFromOil.wrap({p5: 3066638 , wa: 3234882 , p95: 3426069 })
            const energyToEmissionsFactors = isoCombustionEnergyToEmissionFactors.wrap({p5: 71.10, wa: 73.30, p95: 75.50 })
            const expectedResult:Scenarios = {p5:218037962, wa: 237116851, p95: 258668210 }

            const result = isoOilCO2ECombustionEmissions.unwrap(calculateCombustionEmissions(energyFromOil)(energyToEmissionsFactors))
    ...
    describe("Production Emissions", ()=>{
        it(" CO2",()=>{
            const oilProduction = isoOilProduction.wrap(637000)
            const oilProductionCO2Factors = isoOilProductionCO2Factors.wrap({p5: 22.68, wa: 53.08, p95: 106.70 })
            const expectedResult: Scenarios = {p5: 14447160, wa: 33811960, p95: 67967900 } 
            const result = isoOilCO2ProductionEmissions.unwrap(calculateOilCO2ProductionEmissions(oilProduction)(oilProductionCO2Factors))
        ...
        it("CH4  Methane releases ", ()=> {
            const oilProduction = isoOilProduction.wrap(637000)
            const methaneIntensity = isoMethaneIntensity.wrap(0.40)
            const expectedResult = 254800
            const result = isoOilMethaneReleases.unwrap(calculateOilMethaneReleases(oilProduction)(methaneIntensity))
        ...
        it("CH4  CO2E of methane ", ()=>{
            ...
            const methaneFactorisation = isoMethaneFactorisation.wrap(82.50)
            const expectedResult = 21049380
            const result = isoOilCO2EOfMethane.unwrap(calculateOilCO2EOfMethane(methaneReleases)(methaneFactorisation))
        ...
        it(" Upstream Total CO2E",()=>{
            ...
            const expectedResult: Scenarios = {
             ...
             const result = isoOilCO2EProductionEmissions.unwrap(calculateOilCO2EProductionEmissions(oilCO2ProductionEmissions)(oilCO2EOfMethane))
             expect(result.p5.toFixed(0)).toBe(expectedResult.p5.toString())
             expect(result.wa.toFixed(0)).toBe(expectedResult.wa.toString())
             expect(result.p95.toFixed(0)).toBe(expectedResult.p95.toString())
        })
