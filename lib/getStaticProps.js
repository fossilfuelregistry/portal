...
export const getStaticProps = wrapper.getStaticProps( store =>
	async props => {
		...
		try {
			...
			await store.dispatch( { type: 'CO2COSTPERTON', payload: ( await getCO2Costs()?.[ 0 ] || [] ) } )
			await store.dispatch( { type: 'CALCULATION_CONSTANTS', payload: ( await getCalculationConstants() ?? [] ) } )
			await store.dispatch( { type: 'PREFIX_CONVERSIONS', payload: ( await getPrefixConversions() ?? [] ) } )
