...
function CO2ForecastGraphBase({
  ...
  const projectionSourceId = useSelector(
    (redux: Store) => redux.projectionSourceId
  );
  const productionSourceId = useSelector(
    (redux: Store) => redux.productionSourceId
  );
  ...
  useEffect(() => {
    ...
    production.forEach((p) => {
      if (!sourceIds.includes(p.sourceId)) sourceIds.push(p.sourceId);
    });
    ...
    sourceIds.forEach((sourceId) => {
      ...
      settings.supportedFuels.forEach((fuel) => {
        obj[fuel] = false;
        if (fuelsFromSource.find((f) => f === fuel)) obj[fuel] = true;
      });
  ...
  const productionData = combineOilAndGasAndCoal(
    production.filter((d) => d.sourceId === productionSourceId)
  )
  ...
  const projectionData = combineOilAndGasAndCoal(
    projection.filter((d) => d.sourceId === projectionSourceId)
  )
    .filter((d) => d.year >= settings.year.start)
    .map((d) => ({
      year: d.year,
      co2:
        ((d.oil ? sumOfCO2(d.oil.co2, 1) : 0) +
          (d.gas ? sumOfCO2(d.gas.co2, 1) : 0) +
          (d.coal ? sumOfCO2(d.coal.co2, 1) : 0)) *
        costMultiplier,
    }));

  const projProdData = (
    projectionSourceId ? combineOilAndGasAndCoal(projectedProduction) : []
  )
    .filter((d) => d.year >= settings.year.start)
    .map((d) => {
  ...
  const maxCO2 = useMemo(() => {
    const maxInProductionData =
      max(productionData, (d) => (d.oil ?? 0) + (d.gas ?? 0) + (d.coal ?? 0)) ??
      0;
    const maxInProjectionData = max(projectionData, (d) => d.co2) ?? 0;
  ...
  if (!(maxCO2 > 0)) {
    DEBUG && console.info("maxCO2 is less or equal to zero", maxCO2);
    return null; // JSON.stringify( maxCO2 )
  }
        ...
        <table>
          <tbody>
            <tr>
              ...
              <td>
                {getText("gas")}: {getText("past_emissions")}
              </td>
            </tr>
            <tr>
              <td>
              ...
              <td>
                {getText("oil")}: {getText("past_emissions")}
              </td>
            </tr>
            <tr>
              <td>
              ...
              <td>
                {getText("coal")}: {getText("past_emissions")}
              </td>
            </tr>
            {pReserves && (
              <tr>
                ...
                <td>
                  {getText("gas")}: {getText("against_reserves")}
                </td>
              </tr>
            )}
            {pReserves && (
              <tr>
                ...
                <td>
                  {getText("oil")}: {getText("against_reserves")}
                </td>
              </tr>
            )}
            {pReserves && (
              <tr>
                ...
                <td>
                  {getText("coal")}: {getText("against_reserves")}
                </td>
              </tr>
            )}
            {cReserves && (
              <tr>
                ...
                <td>
                  {getText("gas")}: {getText("against_contingent")}
                </td>
              </tr>
            )}
            {cReserves && (
              <tr>
                ...
                <td>
                  {getText("oil")} : {getText("against_contingent")}
                </td>
              </tr>
            )}
            {cReserves && (
              <tr>
                ...
                <td>
                  {getText("coal")} : {getText("against_contingent")}
                </td>
              </tr>
            )}
            {!cReserves && !pReserves && (
